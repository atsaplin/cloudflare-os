...
describe("git-storage migration via the Overseer constructor", () => {
  it("migrates a single-gadget workspace, preserving content across the batching gap",
    ...
    await inOverseer("git-migration-single", async impl => {
      ...
      // running the whole migration ladder: git storage (2), then the pending-action index (3).
...
describe("pending-action-index backfill via the Overseer constructor", () => {
  it("backfills a version-2 workspace's index and stamps version 3", async () => {
    await inOverseer("pending-index-v2", async impl => {
      ...
      // before the pendingByGatekeeper index was declared (its entries only exist for writes
      ...
      // Last write: arm the constructor's version-2 backfill trigger.
    ...
    await inOverseer("pending-index-v2", async impl => {
      expect(impl.storage.version.get()).toBe(3);
      // The index sees exactly the pendings (grouped by gatekeeper, so 1 before 3 here).
      expect([...impl.storage.actions.pendingByGatekeeper.list()].map((r: any) => r.id))
          .toEqual([1, 3]);

      // Resolving a backfilled record must not throw on the index update -- the failure mode
      // that makes this backfill mandatory rather than an optimization.
